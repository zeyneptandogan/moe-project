'''
This script is taken from Pytorch Optimizer library
'''
import torch
...
def _matrix_power_svd(matrix: torch.Tensor, power: float) -> torch.Tensor:
...
def _matrix_power(matrix: torch.Tensor, power: float) -> torch.Tensor: #eigenvalue decomposition 
    # Switch to eigenvalue decomposition (torch.linalg.eigh) instead of SVD for a more stable inversion.
    device = matrix.device
    matrix = matrix.cpu()
    # Use eigenvalue decomposition which is more stable for such matrices
    eigenvalues, eigenvectors = torch.linalg.eigh(matrix)
    # Clamp very small values to avoid instability
    eigenvalues = torch.clamp(eigenvalues, min=1e-12)
    diag_power = torch.diag(eigenvalues.pow(power))
